...
export default function Header() {
  ...
  return (
    <header className="border-b border-gray-300 px-[5%] lg:px-[17%] py-6 sm:py-3 flex flex-col md:flex-row md:justify-between md:items-center gap-y-5 md:gap-y-0 ">
      <Link
        href="/"
        className="text-red-500 font-bold text-4xl md:text-sm font-playfair self-center md:self-baseline"
      >
        Marketer&apos;s Fridge
      </Link>

      {/* 네비게이션 */}
      <nav className="flex flex-wrap justify-center md:justify-end md:gap-x-10 gap-x-[4%] gap-y-2 text-xs text-gray-700">
        {navItems.map((item) => {
